...
Controls: Super+LMB move | Super+RMB resize | Super+Q kill | Hover focus
...
XSelectInput(d,r,PointerMotionMask);
...
for(;;){XNextEvent(d,&e);
 ...
 else if(e.type==MotionNotify&&!s.subwindow&&e.xmotion.subwindow){XGetWindowAttributes(d,e.xmotion.subwindow,&a);
  if(!a.override_redirect&&e.xmotion.subwindow!=w){XRaiseWindow(d,e.xmotion.subwindow);XSetInputFocus(d,w=e.xmotion.subwindow,RevertToPointerRoot,CurrentTime);} }
